...
?>
<!-- Modal toggle -->
<button data-modal-target="kuiz-modal-<?php echo $id_soalan ?>" data-modal-toggle="kuiz-modal-<?php echo $id_soalan?>" class="block text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dankbg-primary-600 dankhover:bg-primary-700 dankfocus:ring-primary-800" type="button">
  Kemaskini Soalan
</button>

<!-- Main modal -->
<div id="kuiz-modal-<?php echo $id_soalan?>" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-md max-h-full">
        ...
        <div class="px-4 pb-4 relative bg-primary-400 rounded-lg shadow-sm dankbg-secondary-700">
            ...
            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dankborder-secondary-600 border-secondary-200">
                ...
                <button type="button" class="text-secondary-400 bg-transparent hover:bg-secondary-200 hover:text-secondary-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dankhover:bg-secondary-600 dankhover:text-white" data-modal-toggle="kuiz-modal-<?php echo $id_soalan?>">
            ...
            <!-- Modal body -->
            <form class="p-4 md:p-5" method="post" action="<?php echo $location_index?>/backend/soalan.php">
                <input type="hidden" name="token" value="<?php echo $token?>">
                <input type="hidden" name="id_soalan" value="<?php echo $soalan_kuiz['id_soalan']?>">
                <div class="grid gap-4 mb-4 grid-cols-2">
                    <div class="col-span-2 pt-2">
                        ...
                        <textarea id="teks_soalan" name="teks_soalan" rows="4" class="block p-2.5 w-full text-sm text-black bg-secondary-50 rounded-lg border border-secondary-300 focus:ring-primary-500 focus:border-primary-500 dankbg-secondary-600 dankborder-secondary-500 dankplaceholder-secondary-400 danktext-white dankfocus:ring-primary-500 dankfocus:border-primary-500" placeholder="Ayat Soalan"><?php echo $soalan_kuiz['teks_soalan']?></textarea>                    
                    </div>
                    ...
                    <div class="">
                        <label for="jawapan_betul" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Jawapan Betul <?php echo $jawapan_betul?></label>
                        <select id="jawapan_betul" name="jawapan_betul" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                            <option value="!" <?= ($jawapan_betul == '!') ? 'selected' : '' ?> >!</option>
                            <option value="," <?= ($jawapan_betul == ',') ? 'selected' : '' ?>>,</option>
                            <option value="." <?= ($jawapan_betul == '.') ? 'selected' : '' ?>>.</option>
                            <option value="?" <?= ($jawapan_betul == '?') ? 'selected' : '' ?>>?</option>
                        </select>
